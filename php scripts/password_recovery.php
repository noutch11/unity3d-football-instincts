...
require_once '/home/u801994192/public_html/Global/connect.php';
require_once "/home/u801994192/public_html/PHPMailer-master/class.phpmailer.php"; 

$emailexists = mysqli_query ($con,"SELECT Email, Password, Username, ID FROM userinfotable WHERE `Email` = '".$Email."'");
...
if (isset ($_POST['recover'])) {
	...
	if ((!empty($Email)) && isset($_POST['email'])) {
		...
		if (mysqli_num_rows ($emailexists) == 0) {
			$errRec = "We have no history of this email address. Please enter an existing one!";
		} else {
			$link = sha1 (generateRandomString() .$Email. $row['Username'] );
			$ins = mysqli_query($con, "INSERT INTO ForgotPass(email,recovery) VALUES('$Email','$link')");
			if ($ins) {
				while ($row = mysqli_fetch_assoc($emailexists)) {
					//mail ($Email, "Your New Password", "Hello, " . $row['Username'] . ". We have recieved an alert about you forgetting your password for your Soccer Instincts account. To change password please visit http://solarwatergames.cf/Accounts/forgotpass.php?recovery=".$link. "&email=" .$Email. " " , "From: contact@solarwatergames.cf");
					
					$mail = new PHPMailer();

					$mail->From = "contact@solarwatergames.cf";
					$mail->FromName = "Solarwater Games Support";

					$mail->AddAddress($Email); // name is optional
					$mail->AddReplyTo ("contact@solarwatergames.cf", "Solarwater Games Support");
					...
					$mail->Body    = "Hello, " . $row['Username'] . ". We have recieved an alert about you forgetting your password for your Soccer Instincts account. <b>To recover your password please visit the password recovery link:</b> <a href=\"http://solarwatergames.cf/Accounts/forgotpass.php?recovery=".$link. "&email=" .$Email. "\">Click Here</a><br><br>*<b>This is an automated email. Please do not reply.</b>*";
					$mail->AltBody = "Hello, " . $row['Username'] . ". We have recieved an alert about you forgetting your password for your Soccer Instincts account. To recover your password please visit http://solarwatergames.cf/Accounts/forgotpass.php?recovery=".$link. "&email=" .$Email. " *This is an automated email. Please do not reply*";
					
					if (!$mail->Send()) {
						 $errRec = "Email could not be sent. <p>";
						 $errRec = "Mailer Error: <b>Unknown error</b>";
						 exit;
					} else
						 $errRec = "Email successfully sent. Please check your email.";
				}
			} else
			 ...
			 $errRec = "Enter a valid email address.";
	}
}
...
<html class="no-js" lang="en">
  <head>
    ...
    <meta name="description" content="Lost you password for a Solarwater Games account? Then all you need to do it enter your email and a new password would be sent to you in no time.">
    <meta name="keywords" content="Solarwater Games, Forgot Password, Password Recovery, Reset Password">
    <meta name="author" content="Solarwater Games">

    <title>Password Recovery - Solar Water Games</title>
    ...
    <section id="main-content">
      ...
      <div class="row">
        <div class="small-16 columns">
          ...
          <p>Lost password? Enter email below and a new password would be sent to you in no time.</p>
      ...
      <div class="row">
        <div class="small-16 large-8 columns">
          <form method="post" action="<?php htmlspecialchars($_SERVER["PHP_SELF"]);?>">
            <p class="error-text"><?php echo $errRec; ?></p>
            <label>
              <input type="text" name="email" placeholder="Email which is associated with your SWG account">
            </label>
            <button type="submit" class="lime-button" style="float:right;" name="recover">Send</button>
          </form>
          <div id="note"></div>
        </div>
        ...
        <div class="small-16 large-7 columns addresses">
          <div class="row small-up-1 medium-up-1">
            ...
            <div class="column">
              ...
              <address>
                <p>
                  If you still having problem loggin into your account, then go to our forums and create a thread in the help section. Remember to create an account on forums, in order to recieve messages privately. <br><br>
                  <a href="#"><button type="button" class="success button" style="font-size:16px;">Account help on Forums</button></a>
                  <a href="../contact.php"><button type="button" class="success button" style="font-size:16px;">Contact Form</button></a>
                </p>
              </address>
            </div>
            <!--//More info-->
